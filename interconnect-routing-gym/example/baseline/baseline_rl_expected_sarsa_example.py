...
for i_episode in range(total_episodes):
	...
	for i in range(1, iter_step):
		print("Baseline Expected Sarsa, i_episode {}; injection_rate {}".format(i_episode, state))
...
csvfile = path + 'Inter_Connect_Networks/Tables/Baseline_ESARSA_'+str(iter_step)+'_'+str(total_episodes)+'.csv'
...
np.savetxt(path + "Inter_Connect_Networks/Rewards_Hist/Baseline_Reward_history_rl_expected_sarsa.csv", rew_history, delimiter=",")
...
plt.xlabel('Episodes')
plt.ylabel('Reward')
plt.savefig(path + 'Inter_Connect_Networks/Figures/Baseline_ESARSA_'+str(iter_step)+'_'+str(total_episodes)+'_ICN.png', bbox_inches='tight')